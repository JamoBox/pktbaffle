...
[**pktcap**](pktcap/) is a companion crate that wraps platform-specific live capture (Linux AF_PACKET, macOS /dev/bpf, Windows Npcap) and pcap/pcapng file I/O behind a unified API. It uses pktbaffle to compile filter expressions before attaching them to the kernel.

See [`pktcap/README.md`](pktcap/README.md) for full documentation.

```toml
[dependencies]
pktcap = { path = "pktcap" }  # or a crates.io version once published
```
